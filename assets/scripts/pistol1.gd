...
enum state { IDLE, CHASE, TELEGRAPH, ATTACK, RETREAT, REPOSITION, STAGGER }
...
func _physics_process(delta: float) -> void:
	...
	match current_state:
		state.IDLE:
			...
			if dist > 50.0:
				pass
			elif cooldown <= 0.0 and dist <= 15.0 and dist > 5.0:
		...
		state.CHASE:
			...
			if los and dist <= 15.0 and cooldown <= 0.0:
				change_state(state.TELEGRAPH)
			elif dist < 5.0:
		...
		state.TELEGRAPH:
			...
			rotate_towards(dir, look_speed * 5.0, delta)
			...
			if telegraph_timer <= 0.0 and cooldown <= 0.0: change_state(state.ATTACK)
